...
class TestLogin:
    def test_login_success_returns_bearer_token(self, client, default_user):
    ...
    def test_login_token_round_trip(self, client, default_user):
        """A token from login must be accepted by a protected route."""
        login = client.post(
            "/api/users/login",
            json={"email": default_user["email"], "password": default_user["password"]},
        )
...
class TestTokenValidation:
    ...
    def test_garbage_token_rejected(self, client, test_db):
        response = client.get(
            "/api/users", headers={"Authorization": "Bearer not.a.real.jwt"}
        )
        assert response.status_code == 401
    ...
    def test_token_for_deleted_user_rejected(self, client, default_user):
        ...
        client.delete("/api/users", headers=default_user["headers"])
        response = client.get("/api/users", headers=default_user["headers"])
        assert response.status_code == 401
...
class TestProfile:
    ...
    def test_update_profile_fields(self, client, default_user):
        ...
        assert response.status_code == 200
        profile = client.get("/api/users", headers=default_user["headers"]).json()
    ...
    def test_update_with_no_fields(self, client, default_user):
        response = client.put("/api/users", json={}, headers=default_user["headers"])
        assert response.status_code == 400
    ...
    def test_password_update_allows_new_login(self, client, default_user):
        response = client.put(
            "/api/users",
    ...
    def test_delete_user(self, client, default_user, test_db):
        response = client.delete("/api/users", headers=default_user["headers"])
        assert response.status_code == 200
...
class TestBatchLookup:
    def test_batch_by_id_preserves_request_order(self, client, make_user):
        ...
        response = client.post(
            "/api/users/id", json={"user_ids": [bob["id"], alice["id"]]}
        )
        assert response.status_code == 200
    ...
    def test_batch_by_id_skips_unknown_ids(self, client, make_user):
        ...
        response = client.post(
            "/api/users/id", json={"user_ids": [alice["id"], unknown]}
        )
        assert response.status_code == 200
        assert len(response.json()) == 1

    def test_batch_by_id_invalid_format(self, client, test_db):
        response = client.post("/api/users/id", json={"user_ids": ["not-an-objectid"]})
        assert response.status_code == 400
    ...
    def test_batch_by_email(self, client, make_user):
        ...
        response = client.post(
            "/api/users/email", json={"emails": [alice["email"]]}
        )
        assert response.status_code == 200
        body = response.json()
        assert len(body) == 1
    ...
    def test_batch_profiles_do_not_leak_password_hash(self, client, make_user):
        alice = make_user(username="alice", email="alice@example.com")
        response = client.post("/api/users/id", json={"user_ids": [alice["id"]]})
...
class TestAvatar:
    ...
    def test_remove_avatar(self, client, default_user, mock_cloudinary):
        ...
        response = client.delete("/api/users/avatar", headers=default_user["headers"])
        assert response.status_code == 200
        profile = client.get("/api/users", headers=default_user["headers"]).json()
...
class TestCascadeDelete:
    def test_delete_removes_owned_experiences(self, client, make_user, test_db):
        ...
        client.delete("/api/users", headers=user["headers"])
    ...
    def test_delete_removes_collaborator_entries(self, client, make_user, test_db):
        ...
        client.delete("/api/users", headers=collaborator["headers"])
        saved = client.get(f"/api/trips/{trip_id}", headers=owner["headers"]).json()
    ...
    def test_delete_unlinks_experiences_from_other_users_trips(
        ...
        client.delete("/api/users", headers=experience_owner["headers"])
        saved = client.get(f"/api/trips/{trip_id}", headers=trip_owner["headers"]).json()
